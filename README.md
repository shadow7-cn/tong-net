...
![同网互通桌面端：互通群聊与文件共享](docs/images/desktop-chat.png)

截图使用演示数据，不包含真实聊天记录或访问凭据。

**[查看全部界面截图](docs/screenshots.md)**：互通服务、群成员、互通记录、虚拟局域网、设置、手机 Web 端，以及 Linux 组网服务管理端。
...
桌面端每次打开时会在后台检查 GitHub 最新正式版本。发现新版后，可点击“前往下载”打开对应版本的发布页面，自行选择安装包；软件不会自动下载或安装更新。选择“稍后再说”后，本次运行不再自动提醒，下次打开会重新检查。

也可以在 **设置 → 软件更新** 中查看当前版本并点击“检查更新”。自动检查失败不会打断使用，手动检查失败时会显示原因。通过浏览器加入群聊的访问端不检查桌面软件更新。
...
3. 点击“连接虚拟局域网”。
4. 连接成功后，在“互通服务”切换到虚拟局域网二维码，或在“互通群聊”复制虚拟局域网地址。
...
- 上传不支持断点续传；下载支持断点续传。
- 当前只提供一个共享群聊，不支持多个群或私聊；跨网络互通需要自行部署同网互通组网服务。
- 不提供用户账号、云同步和远程备份。
